<template>
  <div id="app">
    <div v-if="type == 'nursing'" id="nursingView">
      <div
        class="col-12 col-lg-12 col-md-10 tab typelabel nav-link fixed-nav"
          ...
          <div class="col-sm-7 detail_profile_right">
            <div class="row col-12 pro-heading">
              <div class="col-12 pad-free">
                <h5 class="font15rem font-weight-bold">
        ...
        <div v-if="method_payment.length > 0" class="col-md-12">
          <div class="cost_tb">
            <div class="row col-12 pad-free">
                ...
                <table class="table table-bordered cost_table">
                  ...
                  <tbody>
                    <tr v-for="cost in method_payment" :key="cost.id">
                      ...
                      <td>
                        <span
                          :class="'changeLink changeLink'+cost.id"
                          @click="costConfirm(cost.id)"
                        >詳しくはこちら</span>
                      </td>
                    </tr>
                  </tbody>
              ...
              <div
                class="col-md-12 collapse closeChangeLink"
                :id="'changeLink' + cost.id"
                v-for="cost in method_payment"
                :key="cost.id"
              >
      ...
      <div class="row ele m-lr-0" id="element4">
          ...
          <div class="row col-12">
            <div v-for="maccept in medical_acceptance" :key="maccept.id" class="col-md-4">
              <div class="col-md-12 accept-box">
                <div
                  class="float-left"
                  v-for="(ma,index) in medical"
                  :key="index"
                  style="padding-right:20px;"
                >
                  <i
                    v-if="ma.name === maccept.name && ma.accept_type === 'accept'"
                    class="fas fa-check green"
                  ></i>

                  <i
                    v-if="ma.name === maccept.name && ma.accept_type === 'unaccept'"
                    class="fas fa-times red"
                  ></i>

                  <i
                    v-if="ma.name === maccept.name && ma.accept_type === 'negotiate'"
                    class="fas fa-adjust blue"
                  ></i>
                </div>
                {{maccept.name}}
              </div>
        ...
        <div class="row col-md-12 float: right" style="display: flex; justify-content: flex-end">
          <label for class="m-r-15">
            <i class="fas fa-check green"></i> 受入れ可
          </label>

          <label for class="m-r-15">
            <i class="fas fa-times red"></i> 受入れ不可
          </label>

          <label for class="m-r-15">
            <i class="fas fa-adjust blue"></i> 応相談
          </label>
        </div>
            ...
            <div class="col-12 comment-wrapper">
              <div class="card" v-for="comment in comments" :key="comment.id">
                ...
                <div class="card-body">
                  ...
                  <h5 class="card-title font-weight-bold source-img-small">
                    {{comment.email}}
    ...
    <div v-if="type == 'hospital'" id="hospitalView">
      ...
      <div class="row ele m-lr-0" id="element1">
        ...
        <div class="row col-12 m-b-10">
          ...
          <div class="col-3">
            <div v-for="hospital in hospitals" :key="hospital.id">
              <p>
                <strong class="font-weight-bold">休診日:</strong>
                <font>{{hospital.closed_day}}</font>
              </p>
            </div>
          </div>
          ...
          <div class="row col-12">
            <p>
              <span style="color:red;font-weight:bold;font-size:15px;">※</span>診療時間は、変更される事や、診療科によって異なる場合があるため、直接医療機関のホームページ等でご確認ください。
            </p>
          </div>
        </div>
        ...
        <div class="col-12">
          <div class="row col-md-12">
            <div
              class="col-md-2 fac-name-box"
              v-for="hosfacility in hosfacilities "
              :key="hosfacility.id"
            >
    ...
    if (this.type == "nursing") {
      this.axios.get("/api/profile/nursing/" + this.cusid).then(response => {
        ...
        this.markers[0]["position"]["lat"] =
          response.data.nurselatlong[0]["latitude"];

        this.markers[0]["position"]["lng"] =
          response.data.nurselatlong[0]["longitude"];
        ...
        if (
          response.data.nurselatlong[0]["latitude"] == 0 &&
          response.data.nurselatlong[0]["longitude"] == 0
        ) {
      ...
      this.axios
        .get(`/api/profile/specialfeature/${this.type}/${this.cusid}`)
        .then(response => {
      ...
      this.axios.get("/api/profile/hospital/" + this.cusid).then(response => {
        ...
        this.markers[0]["position"]["lat"] =
          response.data.hoslatlong[0]["latitude"];

        this.markers[0]["position"]["lng"] =
          response.data.hoslatlong[0]["longitude"];
        ...
        if (
          response.data.hoslatlong[0]["latitude"] == 0 &&
          response.data.hoslatlong[0]["longitude"] == 0
        ) {
          this.center["lat"] = 35.6803997;
