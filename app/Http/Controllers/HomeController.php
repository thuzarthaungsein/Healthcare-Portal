...
class HomeController extends Controller
{
    ...
    public function getLatestPostsByAllCatId() {
        ...
        //                 ->get()
        //                 ->toArray();
        $sql = "SELECT categories.name,categories.id,posts.id as pid,posts.title,posts.created_at FROM categories INNER JOIN posts ON categories.id = posts.category_id order by posts.created_at desc";
        // $sql = "select c.name , c.id, group_concat(p.title  order by p.created_at desc limit 6) as title, group_concat(p.photo order by p.created_at desc limit 4) as photo ,group_concat(p.id order by p.created_at desc limit 6) as post_id from categories AS c INNER join posts AS p
