...
$(".path").mouseout(function() {
    ...
    $(".path").on("click", function(e) {
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $(".checkbox, .nursgingcheckbox, .jobcheckbox").addClass(
                    "block2"
                );
            },
            ...
            success: function(data) {
                ...
                $.each(townships, function(k, v) {
                    ...
                    $(".jobcheckbox").append(
                        '<div class="custom-control custom-checkbox col-sm-3"><input name="selector[]" type="checkbox" class="custom-control-input" id="jobcheckbox[' +
                        v.id +
                        ']" value="' +
                        v.id +
                        '"><label class="custom-control-label" for="jobcheckbox[' +
                        v.id +
                        ']">' +
                        v.township_name +
                        "</label></div>"
            ...
            complete: function(data) {
                $(".checkbox, .nursgingcheckbox, .jobcheckbox").removeClass(
                    "block2"
                );
            }
        });
    ...
    function DeltArr(index, type) {
        ...
        var isDivThere = $(
            "#gallery-" + type + " #gallery-" + type + index + ""
        ).index();

        var j_arr = $("#galleryarea-" + type + "" + index + "")
            .attr("class")
            .split("_");
        ...
        for (var i = j_indx; i < photo.length; i++) {
            ...
            $(
                "#gallery-" + type + next_id + " .gallery-area-" + type
            ).removeClass(newClass);

            $("#gallery-" + type + next_id + " .gallery-area-" + type).addClass(
                oldClass
            );
    ...
    */
    ...
    $(function() {
        $("#btnSubmit").on("click", function() {
            ...
            $("#outputpresent").val(
                $("input:checkbox[name=present]:checked").val()
            );
...
function DeltArr(index, type) {
    ...
    for (var i = j_indx; i < photo.length; i++) {
        ...
        $("#gallery-" + type + next_id + " .gallery-area-" + type).removeClass(
            newClass
        );
